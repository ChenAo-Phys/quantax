...
class Euler(Driver):
    ...
    def step(self) -> None:
        ...
        energy = self._optimizer.energy
        if energy is not None:
            self.energy.append(energy, self._time)

        VarE = self._optimizer.VarE
        if VarE is not None:
            self.VarE.append(VarE, self._time)
...
class AdaptiveHeunEvolution(Driver):
    ...
    def step(self) -> None:
        tdvp = self._optimizer
        if not isinstance(tdvp, TimeEvol):
            raise NotImplementedError(
                "`AdaptiveHeunEvolution` is only implemented for `TimeEvol` optimizer."
            )
        dt = self._step_length

        samples = self._sampler.sweep()
        stepi = tdvp.get_step(samples)
        self._state.update(dt * stepi)
        # dtheta0_i = 1j * tdvp.energy - jnp.dot(tdvp._Omean, stepi)

        samples = self._sampler.sweep()
        stepf = tdvp.get_step(samples)
        step1 = (stepi + stepf) / 2
        self._state.update(-dt / 2 * stepi)
        ...
        self._state.update(dt / 2 * stepmm)
        # dtheta0_mm = 1j * tdvp.energy - jnp.dot(tdvp._Omean, stepmm)

        samples = self._sampler.sweep()
        Smat, Fvec = tdvp.get_SF(samples)
        stepff = tdvp.solve_SF(Smat, Fvec)
        self._state.update(dt / 4 * (stepff - stepmm))
        ...
        if tdvp.energy is not None:
            self.energy.append(tdvp.energy, self._time)
        if tdvp.VarE is not None:
            self.VarE.append(tdvp.VarE, self._time)
