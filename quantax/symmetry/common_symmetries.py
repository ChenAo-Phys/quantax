...
def _standardize_coord(coord: NDArray) -> NDArray:
    lattice = get_lattice()
...
def LinearTransform(
    ...
    lattice = get_lattice()
    if center is None:
        _center: NDArray[np.float64] = np.mean(lattice.coord, axis=0)
    else:
        _center = np.asarray(center)

    coord = _standardize_coord(lattice.coord)
    new_coord = np.einsum("ij,nj->ni", matrix, lattice.coord - _center) + _center
